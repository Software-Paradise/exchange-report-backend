  ...
  register: async (req, res) => {
    if (createValidator(req.body)) {
      try {
        ...
        const exist = await database.user.findAll({ where: { EMAIL: user.email } })

        if (!exist[0]) {
          const newUser = {
            EMAIL: user.email,
            PASSWORD: user.password,
            COMISSION: user.commission,
            SHOULDRESETPASS: 1,
      ...
      try {
        ...
        if (user?.[0]) {
          if (bcrypt.compareSync(credentials.password, user[0].PASSWORD)) {
            const token = jwt.sign({ id: `${user[0].IDUSER}`, rol: `${user[0].profile.PROFILE}` }, ACCESS_TOKEN_SECRET)
            res.json({ user: user[0], token, success: true, message: 'User has found' })
          } else {
